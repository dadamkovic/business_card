...
void blinkyLed(uint8_t led_num){
    uint8_t led_mod = led_num % 4;
    switch(led_mod){
        case 0:
...
int main(void)
{
  ...
  while (1)
  {
    ...
    HAL_Delay(300);
    uint8_t song_it = 0;
    Enum_Notes song_note;
    uint8_t song_beats;
    do{
        

        song_note = ode_to_joy_notes[song_it];
        song_beats = ode_to_joy_beats[song_it];
        playNote(song_note, 2, song_beats);
        song_it++;

        blinkyLed((uint8_t) song_note);

    }while(ode_to_joy_beats[song_it] != 0);
